...
class ArrivedVaccine extends Component {
  ...
  ArrivedVaccineCenter(id, arrived_date,vaccine_center_id) {
    this.setState({ loading: true })
    this.state.vaxichain.methods.ArrivedVaccineCenter(id, arrived_date,vaccine_center_id).send({ from: this.state.account })
  ...
  this.ArrivedVaccineCenter = this.ArrivedVaccineCenter.bind(this)
  ...
  render() {
    return (
      ...
      <div id="content" style={{width:650, paddingLeft:530}}>
        <h1 style={{padding:40,width:750,paddingLeft:120}}>Received Vaccine</h1>
        <form onSubmit={(event) => {
          event.preventDefault()
          const id = this.id.value
          const arrived_date = this.arrived_date.value
          const vaccine_center_id = this.vaccine_center_id.value
          
          this.ArrivedVaccineCenter(id, arrived_date,vaccine_center_id)
          ...
          <div className="form-group mr-sm-2" style={{width:650,paddingLeft:170}}>
            <input
              ...
              type="text"
              ref={(input) => { this.vaccine_center_id = input }}
              className="form-control"
              placeholder="VaccineCenter ID"
              required />
          </div>
